...
We can apply this design pattern and refactorDatabaseinto three smaller
...
independently of each other. In the example, the borrow checker knows thata.banda.care distinct and can be borrowed independently, it does not try to
borrow all ofa, which would make this pattern useless.
