...
The result of running theRenameron an AST is a new AST identical to the old
one, but with every name changed tofoo. A real life folder might have some
...
this can be done usingIterator::map. For more complex operations, perhaps
...
nodes are accepted by thefold_*methods.

In the above example we operate onBoxpointers. Since these own their data
...
Iterators have afoldmethod, however this folds a data structure into a
value, rather than into a new data structure. An iterator’smapis more like
this fold pattern.
...
Thevisitorpattern is closely related to fold.
