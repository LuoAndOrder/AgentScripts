...
- Use#[non_exhaustive]onstructs,enums, andenumvariants. For
...
## Alternative:Private fieldsfor structs
...
runtime overhead and prepend_to the field name to avoid the unused field
warning.
...
Onstructs,#[non_exhaustive]allows adding additional fields in a backwards
...
considering if youwantan additional field to be found by clients as a
...
sorts of evolutions are requiredwithoutincrementing the major version.

When#[non_exhaustive]is applied toenums, it forces clients to handle a
...
circumstances. If a client decides topanic!()in this scenario, it may have
been better to expose this error at compile time. In fact,#[non_exhaustive]forces clients to handle the “Something else” case; there is rarely a sensible
