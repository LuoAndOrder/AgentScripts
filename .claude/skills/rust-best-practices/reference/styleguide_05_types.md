...
- unsafe extern "C" fn<'a, 'b, 'c>(T, U, V) -> Worfn()(single spaces around keywords and sigils, and after commas, no trailing commas, no spaces around brackets)
...
- <Baz<T> as SomeTrait>::Foo::BarorFoo::Baror::Foo::Bar(no spaces around::or angle brackets, single spaces aroundas)
...
subsequent lines. When breaking such a type, break beforeevery+:
...
Ause<'a, T>precise capturing bound is formatted as if it were a single path segment with non-turbofished angle-bracketed args, like a trait bound whose identifier isuse.
